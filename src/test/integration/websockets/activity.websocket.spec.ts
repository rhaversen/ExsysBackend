...
describe('Activity WebSocket Emitters', function () {
	...
	beforeEach(async function () {
		...
		const room1 = await RoomModel.create({ name: 'Test Room 1', number: 'TR1', email: 'tr1@example.com' })
		const room2 = await RoomModel.create({ name: 'Test Room 2', number: 'TR2', email: 'tr2@example.com' })
		const room3 = await RoomModel.create({ name: 'Test Room 3', number: 'TR3', email: 'tr3@example.com' })
		room1Id = room1._id.toString()
		room2Id = room2._id.toString()
		room3Id = room3._id.toString()

		const product1 = await ProductModel.create({ name: 'Test Product 1', price: 100, room: room1Id })
		const product2 = await ProductModel.create({ name: 'Test Product 2', price: 200, room: room1Id })
